# 프로젝트 통합 소개 및 브랜치 가이드 — VCE / VulnScanner / AI Scanner

- **프로젝트명**: 클라우드 구축을 통한 취약점 진단 및 모의해킹 프로젝트
- **작성일**: 2026-04-16
- **Main 브랜치 역할**: 전체 프로젝트 구조와 세부 브랜치를 설명하는 통합 README
- **구성 브랜치**: `secure-web` / `vul-web` / `auto-script` / `AI-Scanner`
...
- [AI 자동진단 문서 브랜치 (`AI-Scanner`)](https://github.com/khsqowp/SK_Rookies_FinalPJT/tree/AI-Scanner)

---

## 개요

이 프로젝트는 하나의 결과물만 설명하는 저장소가 아니라, 취약한 웹 서비스의 구현, 취약점 분석과 이행 조치, 취약점 진단 자동화 도구, 그리고 AI 기반 자동진단 체계까지 하나의 흐름으로 묶어 설명하는 통합형 프로젝트입니다. 즉, "취약한 서비스가 어떻게 구성되었는가", "어떤 취약점이 있었는가", "그 취약점을 어떻게 줄였는가", "이 과정을 자동화 도구 관점에서는 어떻게 확장할 수 있는가", "AI 에이전트를 이용해 그 흐름을 어떻게 문서화하고 표준화할 수 있는가"를 함께 보여주는 구조입니다.

전체 흐름은 네 개의 브랜치로 나뉩니다. `vul-web`은 취약점 진단과 모의해킹 실습을 위해 만든 이행 전 취약 웹사이트 브랜치입니다. `secure-web`은 같은 서비스 구조를 바탕으로 취약점을 조치하고 보안 설정을 강화한 이행 후 안전 웹사이트 브랜치입니다. `auto-script`는 OS, 웹서버, DBMS, 클라우드 환경을 대상으로 진단 항목을 자동화하는 취약점 진단 스크립트 및 GUI 도구 브랜치입니다. `AI-Scanner`는 Claude Code를 중심으로 정찰, 퍼징, 검증, 분류, 보고서 작성 과정을 단계별로 수행하도록 설계한 AI 자동진단 문서 브랜치입니다.

따라서 `main` 브랜치는 실제 개발이 이뤄지는 작업 브랜치라기보다, 각 브랜치의 역할과 관계를 설명하는 통합 안내 브랜치로 보는 것이 맞습니다. 취약 웹사이트와 안전 웹사이트를 비교해보려면 `vul-web`과 `secure-web`을 보면 되고, 자동 진단 도구를 확인하려면 `auto-script`를 보면 되며, AI 자동진단 체계를 보려면 `AI-Scanner`를 확인하면 됩니다.
...
| `AI-Scanner` | AI 자동진단 문서 | Claude Code 기반 웹 취약점 자동진단 절차와 보고 체계를 정리한 문서 브랜치 |
...
### `AI-Scanner` — AI 자동진단 문서 브랜치

`AI-Scanner` 브랜치는 실행용 웹 서비스 브랜치가 아니라, Claude Code를 활용해 웹 취약점 자동진단을 어떤 절차로 수행할 것인지 정리한 문서 브랜치입니다. 여기서는 정찰, 공격 벡터별 탐지, 심층 검증, 위험도 분류, 최종 보고서 작성 과정을 Phase 단위로 분리해 정의하고 있습니다.

이 브랜치의 핵심은 AI에게 막연히 취약점을 찾아보라고 지시하는 것이 아니라, 어떤 단계에서 무엇을 해야 하는지와 무엇을 해서는 안 되는지를 명확히 문서화했다는 점입니다. 즉, 자동진단의 정확도와 결과물 품질을 높이기 위해 Claude Code 중심 워크플로우를 작업 기준서 형태로 정리한 브랜치라고 볼 수 있습니다.

주요 구성:

- `Tool_Phase_1.md`: Recon & OSINT
- `Tool_Phase_2.md`: Advanced Fuzzing & Scanning
- `Tool_Phase_3.md`: Deep Logic & Auth Verification
- `Tool_Phase_4.md`: Triage & Threat Modeling
- `Tool_Phase_5.md`: Advanced Reporting
...
| 5단계 | `AI-Scanner`에서 AI 기반 진단 절차와 보고 체계를 문서화 |

이 프로젝트는 단순히 웹사이트 하나를 만드는 흐름이 아니라, 취약한 상태의 서비스에서 시작해 조치가 반영된 서비스로 이동하고, 다시 그 과정을 자동 진단 도구와 AI 진단 체계로 연결하는 흐름을 갖습니다. 이 점이 일반적인 웹 개발 프로젝트와 가장 크게 다른 부분입니다.
...
## 8. AI 자동진단(`AI-Scanner`) 설명

| 항목 | 내용 |
|------|------|
| 프로젝트 성격 | Claude Code 기반 AI 자동진단 문서 브랜치 |
| 적용 범위 | 웹 애플리케이션 및 API 취약점 진단 절차 |
| 핵심 방식 | Phase 기반 정찰, 탐지, 검증, 분류, 보고 |
| 주요 기준 | SK 진단 항목, OWASP Top 10, CWE, CVSS v3.1 |
| 결과물 | Phase 문서, reasoning log 기준, 취약점 보고서 작성 규칙 |

`AI-Scanner` 브랜치는 자동화 스크립트 브랜치와 달리 실행 도구 자체보다 "AI가 어떻게 진단해야 하는가"를 정의하는 문서 브랜치입니다. Claude Code를 중심으로 정찰부터 최종 보고서 작성까지의 워크플로우를 단계별로 나누고, 각 단계에서 허용되는 작업과 금지되는 작업, 검증 방법, 산출물 구조를 구체적으로 정리합니다.

이 브랜치의 의미는 AI에게 단순히 스캔을 시키는 데 있지 않습니다. 어떤 단계에서 무엇을 판단해야 하는지, 어떤 근거를 남겨야 하는지, 후보 취약점과 확정 취약점을 어떻게 구분해야 하는지를 문서화함으로써 AI 진단 결과의 일관성과 재현 가능성을 높이는 데 목적이 있습니다.
...
- `auto-script`와 `AI-Scanner`는 진단 및 문서화 목적의 브랜치이며, 웹 서비스 자체를 제공하는 브랜치는 아닙니다.
